...
library(tidyverse)
library(caret)
>>>>>>> dc24dbf95545460681a1e3286098932e7520eeda
...
ggplot(aes(x=1:35, y=fit, ymin=lwr, ymax=upr), data=interval_big[1:35,]) +
  geom_point() +
  geom_linerange() +
  labs(x="Index of big model", y="Prediction w/ 95% PI") + theme_bw()
...
table <- cbind(bernie_visit[1:2], average_visit[2])
names(table)[3] <- "TD_MLU"
names(table)[2] <- "Bernie_MLU"

table <- table %>%
  mutate(difference = abs(TD_MLU - Bernie_MLU))
...
predict_bernie <- predict(BModel, BernieData[6,], allow.new.levels=T)
predict_bernie - BernieData[6,]$CHI_MLU #0.33
