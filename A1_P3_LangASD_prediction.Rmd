...
#libraries
library(pacman)
p_load(tidyverse, lmerTest, modelr)
...
#the function we will put into rmse 
predictions <- predict(simple, newdata = test, allow.new.levels = T) 
predictions <- predict(big_model, test, allow.new.levels = T) #same as above. 

#with rmse 
test_simple <- Metrics::rmse(test$CHI_MLU, predict(simple, newdata = test, allow.new.levels = T)) # 1.3 
test_rmse <- Metrics::rmse(test$CHI_MLU, predict(big_model, newdata = test, allow.new.levels = T)) # .6
#we have approximately .28 --> .61 

#divide by standard deviation 
test_rmse_by_sd <- test_rmse / sd(test$CHI_MLU) # .68
test_rmse_simple_by_sd <- test_simple / sd(test$CHI_MLU) # .15
...
#create folds - readymade function

#making a for loop. 
for (F in Folds){
  select all the kids in f = testdata
  select all the kids not in f = traindata
}
